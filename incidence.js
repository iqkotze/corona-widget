...
const ENV = {
  ...
  incidenceColors: {
    darkdarkred: {
      limit: 250,
      color: 'incidenceColorsDarkdarkred'
    },
    darkred: {
      limit: 100,
      color: 'incidenceColorsDarkred'
    },
    red: {
      limit: 50,
      color: 'incidenceColorsRed'
    },
    orange: {
      limit: 35,
      color: 'incidenceColorsOrange'
    },
    yellow: {
      limit: 25,
      color: 'incidenceColorsYellow'
    },
    green: {
      limit: 1,
      color: 'incidenceColorsGreen'
    },
    gray: {
      limit: 0,
      color: 'incidenceColorsGray'
    }
  },
  statesAbbr: {
    '8': 'BW',
...
class IncidenceWidget {
  ...
  async createWidget() {
    ...
    if (ENV.isMediumWidget) {
      UIComp.smallIncidenceRow(stateBar, leftCacheID)
    } else {
      UIComp.smallIncidenceBlock(stateBar, leftCacheID)
    }
    stateBar.space(4)
    ...
    if (ENV.isMediumWidget) {
      UIComp.smallIncidenceRow(stateBar, rightCacheID)
    } else {
      UIComp.smallIncidenceBlock(stateBar, rightCacheID)
    }
    stateBar.space(6)
    list.addSpacer(5)
  ...
  async selfUpdate() {
    ...
    let url = 'https://raw.githubusercontent.com/iqkotze/corona-widget/master/incidence.js';
...
class UI {
  ...
  static generateGraph(data, width, height, alignLeft = true) {
    ...
    let max = Math.max.apply(Math, graphData.map(function(o) {
      return o.cases;
    }))
    max = (max <= 0) ? 10 : max;
  ...
  static generateIcidenceGraph(data, width, height, alignLeft = true) {
    ...
    let min = Math.min.apply(Math, graphData.map(function(o) {
      return o.incidence;
    })) / 1.2
...
class RkiRequest {
  async locationData(location) {
    ...
    // https://services7.arcgis.com/mOBPykOjAyBO2ZKk/arcgis/rest/services/RKI_Landkreisdaten/FeatureServer/0/query?where=1%3D1&outFields=${outputFields}&geometry=13.680170%2C51.159111&geometryType=esriGeometryPoint&inSR=4326&spatialRel=esriSpatialRelWithin&returnGeometry=false&outSR=4326&f=json
  ...
  async stateCases(blID) {
    ...
    // https://services7.arcgis.com/mOBPykOjAyBO2ZKk/arcgis/rest/services/RKI_COVID19/FeatureServer/0/query?where=NeuerFall+IN(1,0)+AND+IdBundesland=${blID}+AND+MeldeDatum+>=+TIMESTAMP+'${apiStartDate}'&objectIds=&time=&resultType=standard&outFields=AnzahlFall,MeldeDatum&returnIdsOnly=false&returnUniqueIdsOnly=false&returnCountOnly=false&returnDistinctValues=false&cacheHint=false&orderByFields=MeldeDatum&groupByFieldsForStatistics=MeldeDatum&outStatistics=[{"statisticType":"sum","onStatisticField":"AnzahlFall","outStatisticFieldName":"cases"}]&having=&resultOffset=&resultRecordCount=&sqlFormat=none&f=pjson&token=
  ...
  async dCases() {
    ...
    // https://services7.arcgis.com/mOBPykOjAyBO2ZKk/arcgis/rest/services/RKI_COVID19/FeatureServer/0/query?where=NeuerFall+IN%281%2C0%29+AND+MeldeDatum+%3E%3D+TIMESTAMP+%27${apiStartDate}%27&objectIds=&time=&resultType=standard&outFields=AnzahlFall%2CMeldeDatum&returnIdsOnly=false&returnUniqueIdsOnly=false&returnCountOnly=false&returnDistinctValues=false&cacheHint=false&orderByFields=MeldeDatum&groupByFieldsForStatistics=MeldeDatum&outStatistics=%5B%7B%22statisticType%22%3A%22sum%22%2C%22onStatisticField%22%3A%22AnzahlFall%22%2C%22outStatisticFieldName%22%3A%22cases%22%7D%5D%0D%0A&having=&resultOffset=&resultRecordCount=&sqlFormat=none&f=pjson&token=
    ...
    if (responseToday.status === ENV.status.ok && responseHistory.status === ENV.status.ok) {
      let data = responseHistory.data.features.map(day => {
        return {
          ...
          date: day.attributes.MeldeDatum
        }
      })
...
class Helper {
  static getIncidenceLimits(incidence) {
    if (incidence >= ENV.incidenceColors.green.limit && incidence < ENV.incidenceColors.yellow.limit) {
      return {
        min: ENV.incidenceColors.green.limit,
        max: ENV.incidenceColors.yellow.limit
      }
    } else if (incidence >= ENV.incidenceColors.yellow.limit && incidence < ENV.incidenceColors.orange.limit) {
      return {
        min: ENV.incidenceColors.red.limit,
        max: ENV.incidenceColors.darkred.limit
      }
      ...
      return {
        min: ENV.incidenceColors.darkred.limit,
        max: ENV.incidenceColors.darkdarkred.limit
      }
    } else if (incidence > ENV.incidenceColors.darkdarkred.limit) {
      return {
        min: ENV.incidenceColors.darkdarkred.limit,
        max: 500
      }
    }
    return {
